  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "15px", marginTop: "8%", }}>
        {data.map((el) => (
          <div key={el.id} style={{ border: "2px solid gray", textAlign: "center", width: "350px", height:"650px" }}>
            <p>{el.id}</p>
            ...
            <p>{el.category}</p>
            <Link to={`/Project/${el.id}`}  style={{border:"1px solid black",textDecoration:"none",backgroundColor:"green",color:"white",padding:"6px 50px"}}>ADD-Item</Link>
          </div>
        ))}
      </div>
    </div>
